...
```powershell
.\target\debug\signal-daemon.exe --host 127.0.0.1 --port 8791 --db-path <repo>\dist\signal\signal_demo.db --token dev-token --require-token-for-read --enable-web-push --enable-experimental-actions
```
